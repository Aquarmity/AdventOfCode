...
int main() {
    ...
    infile >> charFiller >> filler >> rgbCode;
    while (infile) {
        int magnitude = toDecimal(rgbCode);
        char direction = toDirection(rgbCode);
        ...
        infile >> direction >> filler >> rgbCode;
    }
    ...
    for (auto i = lagoonPerimeter.begin() + 1; i != lagoonPerimeter.end() + 1; i += 2) {
        if (std::find(yValues.begin(), yValues.end(), i->first) != yValues.end()) {
            if (std::find(yValues.begin(), yValues.end(), (i - 1)->first) != yValues.end()) {
